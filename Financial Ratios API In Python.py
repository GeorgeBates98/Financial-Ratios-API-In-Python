...
def getYahooFinanceData():
    modules, queryBody = yahooFinanceApi()

    # Module 7 'defaultKeyStatistics' data points
    # Converting API response to json
    response = requests.get(queryBody + modules[7])
    data = response.text
    ...
    # Converting API response to json
    response = requests.get(queryBody + modules[8])
    data = response.text
    parsed = json.loads(data)
    ...
    # Converting API response to json
    response = requests.get(queryBody + modules[16])
    data = response.text
    parsed = json.loads(data)
    ...
    # Converting API response to json
    response = requests.get(queryBody + modules[19])
    data = response.text
    parsed = json.loads(data)
    ...
    # Converting API response to json
    response = requests.get(queryBody + modules[26])
    data = response.text
    parsed = json.loads(data)
    ...
    # Converting API response to json
    response = requests.get(queryBody + modules[27])
    data = response.text
    parsed = json.loads(data)
...
def modelScores():
    ...
    insidersBuySellRatio, environmentScore, socialScore, governanceScore, trailingPE = getYahooFinanceData()
    dividendYield, returnOnCapitalEmployed = getFMPData()

    #Value Score:
    valueScore = priceToBook
